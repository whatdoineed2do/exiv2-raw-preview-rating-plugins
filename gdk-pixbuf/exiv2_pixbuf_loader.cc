...
std::unique_ptr<DbgHlpr>  DbgHlpr::_instance = NULL;
...
class Env
{
  public:
    ...
    const Magick::Blob&  srgbICC() const
    { return _srgbICC; }

    const Magick::Blob&  argbICC() const
    { return _argbICC; }
  ...
  private:
    ...
    Magick::Blob  _srgbICC;
    Magick::Blob  _argbICC;
...
static void  _previewImage(const unsigned char* buf_, ssize_t bufsz_, PrevwBuf&  prevwBuf_, std::string& mimeType_)
{
     ...
     * using Magick++
     */
    const unsigned short  PREVIEW_LIMIT = Env::instance().previewScaleLimit();
	...
	if (magick.iccColorProfile().length() == 0)
	{
		...
		d->copy(buf, Exiv2::invalidByteOrder);
                magick.profile("ICC", Magick::Blob(buf, d->size()));
                delete [] buf;
                convert = true;
	    }
	    else
		...
		if ( (d = exif_.findKey(Exiv2::ExifKey("Exif.Nikon3.ColorSpace")) ) != exif_.end())
		{
			...
			if (orig == mod) {
			    // ok, we'll assume this is really still in aRGB
                            magick.profile("ICC", Env::instance().argbICC());
			    convert = true;
			}
	...
	if (convert) {
		...
		magick.profile("ICC", Env::instance().srgbICC());
		magick.iccColorProfile(Env::instance().srgbICC());
	    }
